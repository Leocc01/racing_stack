...
class GlobalRepublisher(Node):
    ...
    def __init__(self):
        super().__init__('global_republisher_node',
        ...
        map_name = ''
        if self.has_parameter('map'):
            map_name = self.get_parameter('map').get_parameter_value().string_value
        if map_name:
            self.get_logger().info(f"Reading parameters from {map_name}")
            try:
                (
                ...
                ) = read_global_waypoints(map_name)
            except FileNotFoundError:
                self.get_logger().warn(f"{map_name} param not found. Not publishing")
        else:
            self.get_logger().info("global_trajectory_publisher did not find any map_name param")

        # Publish at 0.5 Hz (matching the ROS1 rate)
        self.create_timer(2.0, self.global_republisher)
